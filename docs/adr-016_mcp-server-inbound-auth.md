...
- Status: Accepted（「認証方式」の決定のみSuperseded by [[adr-019_mcp-server-builtin-auth]]）
- Date: 2026-07-20
...
「認証方式」（Easy Authを使う、という決定）を[[adr-019_mcp-server-builtin-auth]]でsupersedeした。
公式Azure MCP Serverイメージは`--transport http`起動時に常時アプリ自身でEntra ID受信認証を行う仕様のため、
Easy Authを重ねても二重防御にならないことが判明したため。Ingress方式（external）、リソース側/Backend用
クライアント側App Registrationの発行主体（Issue #8）など、本ADRの他の決定事項は変更していない。
