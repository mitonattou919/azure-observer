...
- Container Apps Ingress配下ではTLSがプラットフォーム側で終端され、コンテナへはHTTPで転送される。
  アプリ内蔵認証が発行する`WWW-Authenticate`チャレンジ等でプロトコルスキームをHTTPSとして
  正しく認識させるため、`AZURE_MCP_DANGEROUSLY_ENABLE_FORWARDED_HEADERS=true`を
  `AZURE_MCP_DANGEROUSLY_DISABLE_HTTPS_REDIRECTION=true`とあわせて設定する
